 ...
 *
 * Namespace: JPKComGutenbergImgAltGitUpdate
 * PHP Version: 8.3+
 ...
 *
 * @package JPKComGutenbergImgAltGitUpdate
 */
    ...
    public function verify_download_checksum( $reply, string $package, \WP_Upgrader $upgrader ) {
        ...
        if ( ! $remote ) {
            return new \WP_Error(
                ...
                __( 'Security verification failed: the update manifest could not be loaded, so the package cannot be verified.', 'jpkcom-gutenberg-img-alt' )
            );
        }
        ...
        if ( empty( $remote->checksum_sha256 ) ) {
            return new \WP_Error(
                'checksum_missing',
                __( 'Security verification failed: the update manifest contains no SHA-256 checksum, so the package cannot be verified.', 'jpkcom-gutenberg-img-alt' )
            );
        }
        ...
        if ( is_wp_error( $temp_file ) ) {
            return new \WP_Error(
                'download_failed',
                sprintf(
                    __( 'Download failed: %s', 'jpkcom-gutenberg-img-alt' ),
        ...
        if ( ! is_string( $calculated_hash ) || ! hash_equals( $expected_hash, $calculated_hash ) ) {
            ...
            $error_msg = sprintf(
                ...
                __( 'Security verification failed: Plugin checksum mismatch. Expected: %1$s, Got: %2$s', 'jpkcom-gutenberg-img-alt' ),
                $expected_hash,
